...
import { Play, Pause, Shuffle, Trash2, ArrowLeft, GripVertical, ListMusic, Edit2, X, Search, Plus, PlusCircle } from 'lucide-react';
...
function PlaylistDetail() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery.trim().length > 2) {
        setIsSearching(true);
        fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(searchQuery)}&entity=song&limit=8`)
          .then(res => res.json())
          .then(data => {
  ...
  const handleDrop = (e, index) => {
    ...
    if (draggedIndex !== index) reorderPlaylist(playlist.id, draggedIndex, index);
  };

  // Modal compartido de búsqueda (usado tanto en Añadir como en Editar)
  const SearchSongs = () => (
    <div className="form-group" style={{ marginTop: '8px' }}>
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          placeholder="Busca un artista, canción..."
          value={searchQuery}
      ...
      {searchResults.length > 0 && (
        <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '300px', overflowY: 'auto' }}>
          {searchResults.map(song => {
            const isAdded = playlist.songs?.some(s => s.trackId === song.trackId);
            return (
              <div key={song.trackId} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', backgroundColor: 'var(--surface-hover)', borderRadius: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', overflow: 'hidden', gap: '12px' }}>
                  <img src={song.artworkUrl100} alt={song.trackName} style={{ width: '40px', height: '40px', borderRadius: '6px', flexShrink: 0 }} />
                  <div style={{ overflow: 'hidden' }}>
                    <div style={{ fontSize: '14px', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{song.trackName}</div>
                ...
                {!isAdded ? (
                  <button
                    ...
                    style={{ background: 'var(--accent-color)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', cursor: 'pointer', flexShrink: 0, marginLeft: '8px' }}
                  >
                    <Plus size={16} />
                  </button>
                ) : (
                  <span style={{ fontSize: '12px', color: 'var(--accent-color)', marginLeft: '8px', flexShrink: 0 }}>✓ Añadida</span>
                )}
              </div>
            );
          })}
      ...
      {searchQuery.trim().length > 0 && searchQuery.trim().length <= 2 && (
        <p style={{ color: 'var(--text-secondary)', fontSize: '12px', marginTop: '8px' }}>Escribe al menos 3 caracteres...</p>
      )}
    </div>
  );
  ...
  return (
    <div className="page-container">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '24px', marginBottom: '32px' }}>
        <div style={{ width: '200px', height: '200px', backgroundColor: 'var(--surface-color)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.3)', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '32px', flexWrap: 'wrap' }}>
        ...
        <button className={`btn-secondary ${isShuffle ? 'active' : ''}`} onClick={toggleShuffle} style={isShuffle ? { color: 'var(--accent-color)' } : {}}>
          <Shuffle size={20} /> Aleatorio
        </button>
        {/* ✅ Botón dedicado para añadir canciones */}
        <button className="btn-secondary" onClick={handleOpenAdd}>
          <PlusCircle size={20} /> Añadir canciones
        </button>
      ...
      {playlist.songs?.length > 0 ? (
          ...
          {playlist.songs.map((song, index) => {
            ...
            return (
              <div
                ...
                style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', backgroundColor: 'var(--surface-color)', borderRadius: '8px', cursor: 'pointer', border: isThisPlaying ? '1px solid var(--accent-color)' : '1px solid transparent', transition: 'background-color 0.2s' }}
        ...
        <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-secondary)' }}>
          <ListMusic size={48} style={{ marginBottom: '16px', opacity: 0.4 }} />
          <p style={{ marginBottom: '16px' }}>No hay canciones en esta playlist.</p>
          <button className="btn-primary" onClick={handleOpenAdd}>
            <Plus size={18} /> Añadir tu primera canción
          </button>
        </div>
      )}
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="form-container" style={{ position: 'relative', width: '100%', maxWidth: '480px' }}>
            <button onClick={() => setShowAddModal(false)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
              <X size={24} />
            </button>
            <h2 className="form-title">Añadir canciones</h2>
            <SearchSongs />
          </div>
        </div>
      )}
      ...
      {showEditModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="form-container" style={{ position: 'relative', width: '100%', maxWidth: '400px' }}>
            <button onClick={() => setShowEditModal(false)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
              <X size={24} />
            </button>
